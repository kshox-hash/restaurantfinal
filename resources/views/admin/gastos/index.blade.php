        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body"> 
                    <div class="card">
                        <div class="card-body">  
                            <form class="row" action="{{route('gastos.create')}}" method="POST"  enctype="multipart/form-data"> 
                                @csrf
                                ...
                                <div class="row"> 
                                    ...
                                    <div class="col-md-12"> 
                                        <div class="input-group input-group-sm mb-3"> <span class="input-group-text" for="telefono">COSTO:</span>
                                            <input type="text" name="costo" id="costo" class="form-control" required aria-label="Sizing example input" aria-describedby="inputGroup-sizing-sm">
                                        </div> 
                                    </div>   
                                    <br>
                    ...
                    <tbody> 
                        @foreach ($gastos as $row)
                            <tr>
                                <td>{{$row->id}}</td> 
                                <td> 
                                    <textarea style="border:none" rows="1" class="form-control">{{$row->descripcion}}</textarea> 
                                </td> 
                                <td>{{$row->costo}}</td> 
                                <td>
                                    <a onclick="Editar({{$row->id}},`{{$row->descripcion}}`,'{{$row->costo}}')" data-bs-toggle="modal" data-bs-target="#modaluser" class="btn btn-success">Editar</a>
                                    <a href="{{route('gastos.delete',$row->id)}}" class="btn btn-danger">Eliminar</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>  
